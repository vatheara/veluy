...
const createErrorResponse = (errorCode: number, responseCode: number, message: string): BaseResponse => ({
  responseCode,
...
app.post('/v1/renew_token', async (c) => {
  try {
    ...
    return c.json(createErrorResponse(9, 1, 'Cannot connect to server. Please try again later.'), 500)
  }
})
...
app.post('/v1/generate_deeplink_by_qr', async (c) => {
  try {
    ...
    if (qr.includes('static')) {
      return c.json(createErrorResponse(2, 1, 'Sorry, the system does not support static QR code.'), 400)
    }
    ...
    return c.json(createSuccessResponse(deeplinkData, 'Getting deep link successfully'))
  } catch (error) {
    return c.json(createErrorResponse(4, 1, 'Error occurred on requesting deeplink from provider.'), 500)
  }
})
...
app.post('/v1/check_transaction_by_md5', authMiddleware, async (c) => {
  try {
    ...
    if (!transaction) {
      return c.json(createErrorResponse(1, 1, 'Transaction could not be found. Please check and try again.'), 404)
    }

    return c.json(createSuccessResponse(transaction, 'Getting transaction successfully.'))
  } catch (error) {
...
app.post('/v1/check_transaction_by_short_hash', authMiddleware, async (c) => {
  try {
    ...
    if (hash.length !== 8) {
      return c.json(createErrorResponse(5, 1, 'Hash must be 8 characters long.'), 400)
    }

    if (!['USD', 'KHR'].includes(currency)) {
      return c.json(createErrorResponse(5, 1, 'Currency must be USD or KHR.'), 400)
    }

    // Mock: find transaction by short hash
    const fullHash = Object.keys(mockTransactions).find(key => key.startsWith(hash))
    if (!fullHash) {
      return c.json(createErrorResponse(1, 1, 'Transaction could not be found. Please check and try again.'), 404)
    }
...
app.post('/v1/check_transaction_by_instruction_ref', authMiddleware, async (c) => {
  try {
    const body = await c.req.json()
...
app.post('/v1/check_transaction_by_md5_list', authMiddleware, async (c) => {
  try {
    ...
    if (md5List.length > 50) {
      return c.json(createErrorResponse(5, 1, 'Maximum 50 items allowed.'), 400)
    }
    ...
    return c.json(createSuccessResponse(results, 'Getting transaction data successfully.'))
  } catch (error) {
    return c.json(createErrorResponse(9, 1, 'Cannot connect to server. Please try again later.'), 500)
  }
})
